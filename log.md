...
### 作業内容
- `README.md` にデプロイ・スマホ実機テスト向けの記述を追記
  - セットアップ「方法2」に、同一 Wi-Fi のスマホから PC の IP でアクセスする手順を追記
  - セットアップ「方法3: Cloudflare Pages で公開する」を新設（ビルドなし・ルート公開の設定表、GitHub Pages でも同様に公開可能な旨を併記）
...
### 備考
- 本アプリはルート直下が静的ファイル構成のため、Cloudflare Pages / GitHub Pages ともにビルド工程なしで公開できる
